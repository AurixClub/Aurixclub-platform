...
export async function GET(req: NextRequest) {
  ...
  const { response, status } = await programController.handleList(getToken(req), { department_id: searchParams.get("department_id") || undefined });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { response, status } = await programController.handleCreate(getToken(req), body);
    return NextResponse.json(response, { status });
  } catch {
