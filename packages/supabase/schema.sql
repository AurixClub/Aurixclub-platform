...
-- =============================================================================
-- Copy of packages/supabase/migrations/00001_initial_schema.sql
-- Paste and run this SQL directly in your Supabase Dashboard SQL Editor.
-- =============================================================================
...
-- 4. TEAM MEMBERS (Founders & Core Leadership)
...
CREATE POLICY "Users can update own profile" ON public.profiles FOR UPDATE USING (id = current_setting('request.jwt.claim.sub', true));
CREATE POLICY "Active departments are readable" ON public.departments FOR SELECT USING (status = 'active');
CREATE POLICY "Department members are readable" ON public.department_members FOR SELECT USING (true);
CREATE POLICY "Visible team members are readable" ON public.team_members FOR SELECT USING (is_visible = true);
CREATE POLICY "Published events are readable" ON public.events FOR SELECT USING (status = 'published');
CREATE POLICY "Users can view own registrations" ON public.event_registrations FOR SELECT USING (user_id = current_setting('request.jwt.claim.sub', true));
CREATE POLICY "Users can register for events" ON public.event_registrations FOR INSERT WITH CHECK (user_id = current_setting('request.jwt.claim.sub', true));
CREATE POLICY "Users can view own applications" ON public.applications FOR SELECT USING (user_id = current_setting('request.jwt.claim.sub', true));
...
CREATE POLICY "Published programs are readable" ON public.programs FOR SELECT USING (status = 'published');
